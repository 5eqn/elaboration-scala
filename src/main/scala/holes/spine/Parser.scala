...
object ScalaParser extends RegexParsers {
  ...
  def pLam: Parser[Raw] = for {
    ...
    _ <- char('.')
    t <- pRaw
  } yield xs.foldRight(t)(Raw.Lam.apply)
  ...
  def pPi: Parser[Raw] = for {
    ...
    params.foldRight(t)((param, acc) => Raw.Pi(param, a, acc))
  }

  def funOrSpine: Parser[Raw] = pSpine.flatMap { sp =>
    opt(pArrow).flatMap {
      case Some(_) => pRaw.map(t => Raw.Pi("_", sp, t))
